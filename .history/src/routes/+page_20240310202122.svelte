		...
		>
			{#each tags as tag, index (tag)}
				...
				>
					{tag}
					<span
						class="cursor-pointer h-full bg-r text-[var(--text-color)] overflow-clip rounded-[0.27rem] px-[0.24rem] py-[auto] text-right text-[0.75em] font-bold leading-none text-white"
						tabindex="0"
						role="button"
						on:click={() => remove(tag)}
						on:keydown={(e) =>
