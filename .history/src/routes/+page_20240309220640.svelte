	...
	<label for="add-tag-input" class="tag-input">
		...
		>
			{#each tags as tag, index (tag)}
				<li
					class="max-h-[36px] overflow-hidden inline-block whitespace-nowrap rounded-[0.27rem] bg-primary-100 px-[0.65em] pb-[0.25em] pt-[0.35em] text-center align-baseline text-[0.75em] font-bold leading-none text--[var(--tag-text-color)] border-2"
				>
					{tag}
					<span
						class="cursor-pointer bg-sky-300"
						tabindex="0"
						role="button"
						on:click={() => remove(tag)}
