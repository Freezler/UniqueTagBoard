	...
	>
		<label for="add-tag-input" class="tag-input">
			<ul
				...
				class="overflow-scroll scroll-pe-44 flex flex-row flex-wrap items-center justify-cbetween w-[85vw] bg-pink-950/5 gap-2 h-[48vh] p-8"
			>
				{#each tags as tag, index (tag)}
				
					<li
						class="flex items-center justify-center  w-[100px] h-[20px] text-[var(--text-color)] bg-[var(--btn-bg-color)] rounded-[0.27rem] text-[14px] font-bold leading-[14px] box-border"
					>
						{tag}
						<span
							class="z-100 w-[20px] right-0 cursor-pointer h-full bg-[var(--bg-color)] text-[var(--text-color)]
	 						rounded-tr-[0.27rem] rounded-br-[0.27rem] text-[24px] font-thin
