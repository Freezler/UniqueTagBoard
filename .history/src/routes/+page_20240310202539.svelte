	...
	<label for="add-tag-input" class="tag-input">
		...
		>
			{#each tags as tag, index (tag)}
				<li
					class="grid max-h-[1.9rem] grid-cols-[1fr_20px] overflow-hidden whitespace-nowrap rounded-[0.27rem] bg-primary-100
				...
				>
					{tag}
					<span
						class="ml-4 cursor-pointer h-full bg-[var(--bg-color)] text-[var(--text-color)] overflow-clip rounded-[0.27rem] px-[0.24rem] py-[auto] text-right text-[0.75em] font-bold leading-none text-white"
						tabindex="0"
						role="button"
						on:click={() => remove(tag)}
						on:keydown={(e) =>
