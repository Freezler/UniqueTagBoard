<script>
	...
	function addItem(tagInput) {
		...
		tags.update((existingTags) => {
			if (!existingTags.includes(value)) {
				return [...existingTags, value].sort((a, b) =>
					a.localeCompare(b)
				);
			} else {
	...
	document.getElementById('add-tag-input').addEventListener('keydown', function (e) { if (e.key === 'Enter') { addItem(); } }); document.getElementById('add-tag-submit').addEventListener('click', addItem);
