<script>
	...
	// const btn = document.getElementById('add-tag-button');

	function addItem(e) {
		if (e.which === 13 || btn.clicked === true) {
	...
	function handleButtonClick() {
		...
		addTagButton.addEventListener('click', addItem);
	}
...
<body class="bg-slate-950 h-[100vh]">
	...
	>
		<label for="add-tag-input" class="tag-input">
			...
			<div
				class="grid place-items-center pb-0 bg-slate-900"
			>
				<h1
				...
				>
					project made with sveltekit, tailwind and
					autoanimate.
				</p>
			</div>
			<div
			...
			>
				...
				<form action="">
					...
					<button
						type="submit"
						id="add-tag-button"
						on:click|preventDefault={addItem}>Add</button
					>
				</form>
			</div>
		</label>
