		...
		>
			{#each tags as tag, index (tag)}
				...
				>
					{tag}
					<span
						class=" cursor-pointer h-full bg-[var(--bg-color)] text-[var(--text-color)] align-baseline rounded-[0.27rem] ml-[] py-[auto]  text-[0.75em] font-bold leading-none text-white"
						tabindex="0"
						role="button"
						on:click={() => remove(tag)}
						on:keydown={(e) =>
