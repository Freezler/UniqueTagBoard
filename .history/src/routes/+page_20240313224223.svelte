	...
	>
		<label for="add-tag-input" class="tag-input">
			<ul
				...
				class="overflow-scroll flex flex-row flex-wrap items-center justify-center w-[85vw] bg-pink-950/5 gap-2 h-[48vh] p-8"
			>
				{#each tags as tag, index (tag)}
				
					<li
						class="flex-1 items-center justify-center  pl-2 pr-2  h-[24px] text-[var(--text-color)] bg-[var(--btn-bg-color)] rounded-[0.27rem] text-[14]6px] font-bold leading-[14px] box-border"
					>
						{tag}</li>
						<span
							class="flex-1 z-100  pl-2 pr-2  h-[32px] bg-[var(--bg-color)] text-[var(--text-color)]  right-0
