		...
		<label for="add-tag-input" class="tag-input">
			...
			>
				{#each tags as tag, index (tag)}
					<li
						class="text-[16px] grid grid-cols-[1fr_32px] place-items-center box-border rounded-[0.27rem] bg-[var(--bg-color)] object-cover
					    text-start w-[calc(content+62px)] font-semibold leading-none text-[var(--text-color)] border-[1px] border-cyan-200 pl-2  pr-2 mb-1"
					>
						{tag}
						<span
							class="relative w-4 cursor-pointer h-full bg-[var(--btn-bg-color)] text-[var(--text-color)]
	 						rounded-tr-[0.27rem] rounded-br-[0.27rem] m5-[5rempx] text-[28px] font-thin leading-none
