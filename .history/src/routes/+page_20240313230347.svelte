		...
		<label for="add-tag-input" class="tag-input">
			...
			>
				{#each tags as tag, index (tag)}
					<div class="flex items-center box-border border-2 rounded-[0.26rem]">
						<li class=" h-[32px] bg-[var(--btn-bg-color)] text-[var(--text-color)] pl-2 pr-2">
							{tag}
						</li>
						<span
							class=" h-[32px] box-border bg-blue-500 text-center justify-center align-middle text-[var(--text-color)] rounded-tr-[0.25rem] rounded-br-[0.25rem] w-[30px] -translate-y-1/2"
							tabindex="0"
							role="button"
							on:click={() => remove(tag)}
							on:keydown={(e) =>
