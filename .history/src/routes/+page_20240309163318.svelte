...
<label for="add-tag-input" class="tag-input">
	<ul class="flex flex-row flex-wrap gap-4 justify-around ">
		...
		{#each tags as tag (tag)}
			<li use:autoAnimate={{ duration: 300, easing: 'ease-in-out' }} class="tag border-2 border-black rounded-lg w-28 text-center">{tag}<button class="pl-1 " on:click={() => remove(tag)}>x</button></li>
		{/each}
	</ul>
