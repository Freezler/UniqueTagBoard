	...
	>
		<label for="add-tag-input" class="tag-input">
			<ul
				...
				class="flex flex-row flex-wrap items-center justify-center w-[80vw] bg-slate-900 gap-4 h-[60vh] mb-4 p-2 overflow-auto"
			>
				{#each tags as tag, index (tag)}
					<li
						class="relative text-[16px] grid grid-cols-[1fr_32px] place-items-center box-border rounded-[0.27rem] bg-[var(--bg-color)] object-cover
					    text-start w-[calc(content+62px)] font-semibold leading-none text-[var(--text-color)] border-[1px] border-cyan-200 pl-2  pr-2 mb-1"
					>
						{tag}
						<span
							class="absolute w-4  right-0cursor-pointer h-full bg-[var(--btn-bg-color)] text-[var(--text-color)]
	 						rounded-tr-[0.27rem] rounded-br-[0.27rem] text-[28px] font-thin leading-none
						  text-amber-50 selection:border-cyan-100 pb-1 pl-5"
							tabindex="0"
							role="button"
							on:click={() => remove(tag)}
							on:keydown={(e) =>
			...
			>
				<form action="" class="flex gap-1">
					...
					<button
						class="border-0 border-[var(--bg-color)] align-middle rounded-[0.27rem] text-white relative"
						type="button"
						id="add-tag-submit"
